    ...
    public class ReceiptService : IReceiptService
    {
        ...
        public async Task<List<ReceiptDto>> GetAllAsync()
        {
            ...
            return receipts.Select(receipt => new ReceiptDto
            {
                ...
                CompanyName = receipt.Payment?.Debt?.Customer?.CompanyName ?? "",
                Amount = receipt.Payment?.Amount ?? 0,
                PaymentDate = receipt.Payment?.PaymentDate ?? DateTime.MinValue,
                FeeTypeName = receipt.Payment?.FeeType?.Name ?? "",
                Description = receipt.Payment?.Description ?? "",



SignedFileName = receipt.SignedFileName,
                SignedFilePath = receipt.SignedFilePath,
                SignedUploadedDate = receipt.SignedUploadedDate
            }).ToList();
        }
        ...
        public async Task<ReceiptDto> AddAsync(CreateReceiptDto dto)
        {
            ...
            var existingReceipt = await _receiptRepository.GetByPaymentIdAsync(dto.PaymentId);
            ...
            if (existingReceipt != null)
            {
                return new ReceiptDto
                {
                    ...
                    SignedFileName = existingReceipt.SignedFileName,
                    SignedFilePath = existingReceipt.SignedFilePath,
                    SignedUploadedDate = existingReceipt.SignedUploadedDate
                };
            }
            ...
            var receipt = new Receipt
            {
                ...
                ReceiptNumber = $"MKB-{DateTime.Now:yyyyMMddHHmmssfff}",
        ...
        public async Task<bool> UpdateAsync(int id, UpdateReceiptDto dto)
        {
            var receipt = await _receiptRepository.GetByIdAsync(id);
        ...
        public async Task<bool> UploadSignedReceiptAsync(int id, IFormFile file)
        {
            var receipt = await _receiptRepository.GetByIdAsync(id);
        ...
        ".png"
            ...
            if (!allowedExtensions.Contains(extension))
                throw new Exception("Sadece PDF, JPG, JPEG ve PNG dosyaları yüklenebilir.");

            const long maxFileSize = 10 * 1024 * 1024;

            if (file.Length > maxFileSize)
                throw new Exception("Dosya boyutu 10 MB'dan büyük olamaz.");
            ...
            receipt.SignedFileName = file.FileName;

            receipt.SignedFilePath =
                $"/uploads/signed-receipts/{uniqueFileName}";

            receipt.SignedUploadedDate = DateTime.Now;
